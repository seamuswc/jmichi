...
if ! pm2 reload jmichi --update-env; then
    echo "❌ Failed to reload application!"
    echo "💡 Trying to restart application..."
    if ! pm2 restart jmichi --update-env; then
...
ATTEMPTS=0
MAX_ATTEMPTS=30
until pm2 jlist | jq -e '.[] | select(.name=="jmichi" and .pm2_env.status=="online")' >/dev/null 2>&1; do
    ATTEMPTS=$((ATTEMPTS+1))
    if [ "$ATTEMPTS" -ge "$MAX_ATTEMPTS" ]; then
        echo "❌ Application is not reported online by PM2"
        echo "📝 PM2 list:" && pm2 list || true
        echo "📝 Recent logs:" && pm2 logs jmichi --lines 20 || true
        exit 1
    fi
    sleep 1
done
...
ATTEMPTS=0
MAX_ATTEMPTS=10
until curl -sf --max-time 5 "http://localhost" | grep -qE "(Jmichi|<div id=\"root\"|<!doctype html)"; do
    ATTEMPTS=$((ATTEMPTS+1))
    if [ "$ATTEMPTS" -ge "$MAX_ATTEMPTS" ]; then
...
echo "   View logs: pm2 logs jmichi"
