...
function AdvancedDataPage() {
    ...
    if (chartRef.current) {
      ...
      const chartInstance = new Chart(ctx, {
        ...
        data: {
          labels: labels,
          datasets: [{
            label: chartType === 'price' ? `Average Price (JPY)` : 'Listings Volume',
  ...
  const switchChart = (type) => {
    setChartType(type);
    if (chart) {
      chart.data.datasets[0].label = type === 'price' ? `Average Price (JPY)` : 'Listings Volume';
