...
function CreatePage() {
  const [rentalType] = useState('living'); // Always 'living'
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <p className="text-sm text-yellow-700 mt-1">
                <strong>メンテナンス中です。支払いやリスティングを行わないでください。ご理解ありがとうございます。</strong>
              </p>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={(e) => e.preventDefault()} noValidate>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Coordinates (Latitude, Longitude) / 座標 *
                  </label>
                  <input
                    type="text"
                    name="coordinates"
                ...
                <div>
                  ...
                  <div className="mt-3">
                    <div className="flex flex-wrap gap-2 mb-3">
                      <button
                        type="button"
                        onClick={() => translateDescription('japanese')}
            ...
            <div className="mb-6 md:mb-8">
              <h2 className="text-lg md:text-2xl font-semibold text-gray-800 mb-4 md:mb-6 flex items-center">
                ...
                Enter code to create listing / コードを入力してリスティングを作成
              </h2>
              ...
              <div className="bg-gradient-to-r from-yellow-50 to-orange-50 p-4 md:p-6 rounded-xl border border-yellow-200">
                ...
                <p className="text-xs text-gray-600 mt-2">
                  Have a promo code? Enter it to skip payment / プロモコードをお持ちですか? 支払いをスキップするために入力してください
                </p>
              </div>
            </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-gray-700">
                      Choose Payment Network / 支払いネットワークを選択 *
                    </label>
                  ...
                  <div className="mt-6">
                    ...
                    >
                      {isSubmitting ? '⏳ Processing...' : '🎟️ Buy Promo Code with Solana / Solanaでプロモコードを購入'}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800 text-center">
                ⚠️ Listings are automatically deleted after 3 months / リスティングは3ヶ月後に自動的に削除されます
              </p>
            </div>
      ...
      {showCoordinateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold mb-4">📍 Building Name Conflict / 建物名の競合</h3>
            <p className="text-gray-600 mb-4">
              We found existing buildings at these coordinates. Would you like to: / これらの座標に既存の建物が見つかりました。次を選択してください:
            </p>
            
            <div className="space-y-3">
              <div className="text-sm font-medium text-gray-700">Use existing building name: / 既存の建物名を使用:</div>
              {existingBuildings.map((buildingName, index) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-500">Keep existing name / 既存の名前を保持</div>
                </button>
              ))}
              ...
              <div className="border-t pt-3">
                <button
                  ...
                  className="w-full text-left p-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
                >
                  <div className="font-medium text-gray-700">Use new building name / 新しい建物名を使用</div>
                  <div className="text-sm text-gray-500">Enter a more accurate name / より正確な名前を入力</div>
                </button>
              </div>
            </div>
            
            <div className="mt-4 text-xs text-gray-500">
              💡 Tip: Using the same building name will group all listings together on the map / ヒント: 同じ建物名を使用すると、すべてのリスティングが地図上でグループ化されます
            </div>
          </div>
        </div>
      )}
