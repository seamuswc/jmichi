...
function MapPage() {
  ...
  }, [filterPool, filterParking, filterTopFloor, minSqm, maxSqm, minPrice, maxPrice, setSearchParams]);
  ...
  // SEO Content
  const seoTitle = `Japan Rental Property Search | Find Apartments, Condos & Houses in Japan`;
  const seoDescription = `Search ${totalListings > 0 ? `${totalListings}+ ` : ''}rental properties in Japan. Find apartments, condos, houses, and commercial spaces for rent. Average price ${avgPrice > 0 ? `${avgPrice.toLocaleString()}¥/month` : 'available'}. Filter by price, size, pool, parking, and more.`;
  const seoKeywords = `Japan rentals, Japan apartments, Japan condos, Japan houses, rent in Japan, Japan property rental, Japan accommodation, Japan real estate, apartments for rent Japan, condos for rent Japan, long term rental Japan, 日本 賃貸, 日本 アパート, 日本 マンション`;
  ...
  return (
    <>
      ...
      <div className="absolute top-2 left-2 md:top-4 md:left-4 z-10 flex flex-col gap-2">
        ...
        >
          {showFilters ? 'Hide / 非表示' : 'Filters / フィルター'}
        </button>
        {showFilters && (
        <div className="bg-white/95 backdrop-blur rounded-lg shadow p-3 md:p-4 w-72 space-y-2 border">
          <div className="text-sm font-semibold mb-1">Filters / フィルター</div>
          ...
          <button
            ...
            onClick={() => { setMinSqm(''); setMaxSqm(''); setMinPrice(''); setMaxPrice(''); setFilterPool(false); setFilterParking(false); setFilterTopFloor(false); setFilterSixMonths(false); }}
          >
            Clear / クリア
          </button>
        </div>
