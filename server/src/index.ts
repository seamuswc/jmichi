...
app.get('/sitemap.xml', async (request, reply) => {
  try {
    const hostname = request.headers.host || 'jmichi.com';
...
const authenticateToken = (request: any, reply: any, done: any) => {
  ...
  if (!expectedToken) {
    return reply.code(500).send({ error: 'Admin token not configured' });
  }
...
app.get('/api/cities/listings', async (request, reply) => {
  try {
    // Count Japan listings (latitude 24.0-46.0, longitude 122.0-146.0)
    const japanListings = await prisma.listing.count({
      where: {
...
app.post('/api/translate', async (request, reply) => {
  try {
    ...
    const languageMap: { [key: string]: string } = {
      ...
      'russian': 'Russian',
      'korean': 'Korean',
      'thai': 'Thai' // Keep for backwards compatibility
    };
...
app.get('/api/settings/current', { preHandler: authenticateToken }, async (request, reply) => {
  try {
    ...
    const currentSettings = {
      ...
      emailSender: process.env.TENCENT_SES_SENDER || 'data@jmichi.com',
